"""Provides the Coordinator classes for polling blackout periods."""
...
from .entities import BlackoutPeriod, PowerOffPeriod
...
class VitaBlackoutsCoordinator(DataUpdateCoordinator):
    """Coordinates the polling of blackout periods."""
    ...
    async def _async_update_data(self) -> dict:
        ...
        try:
            ...
            LOGGER.exception("Cannot obtain blackout periods for group %s", self.group)
            msg = f"Blackouts not polled: {err}"
            raise UpdateFailed(msg) from err

    async def _fetch_periods(self) -> None:
        self.periods = await self.api.get_blackout_periods()
